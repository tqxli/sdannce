...
class COMTrainer(DANNCETrainer):
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs + 1):
            # open csv
            stats_file = open(
                os.path.join(self.params["com_train_dir"], "training.csv"),
                "a",
                newline="",
            )
            ...
            result_msg = f"Epoch[{epoch}/{self.epochs}] " + "".join(
                f"train_{k}: {val:.6f} " for k, val in train_stats.items()
            )
            ...
            if len(valid_stats) != 0:
                ...
                result_msg = result_msg + "".join(
                    f"val_{k}: {val:.6f} " for k, val in valid_stats.items()
                )
    ...
    def _train_epoch(self, epoch):
        ...
        for batch in pbar:
            ...
            result = f"Epoch[{epoch}/{self.epochs}] " + "".join(
                f"train_{loss}: {val:.6f} " for loss, val in loss_dict.items()
            )
            ...
            if len(self.metrics.names) != 0:
                metric_dict = self.metrics.evaluate(
                    pred.detach().cpu().numpy(), gt.clone().cpu().numpy()
                )
